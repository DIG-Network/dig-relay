...
impl PexRelay {
    ...
    pub fn known_count(&self, network_id: &str) -> usize {
        self.engines
            .get(network_id)
            .map_or(0, PexEngine::known_count)
    }
...
mod tests {
    ...
    fn snapshot_is_scoped_to_the_links_network() {
        ...
        assert_eq!(
            ids,
            vec![b.clone()],
            "only same-network peers; Z (netY) excluded"
        );
    ...
    fn a_new_registration_surfaces_as_an_added_delta_on_tick() {
        ...
        // Tick past the effective interval + max jitter (60 s + ≤10%) so eligibility is
        // deterministic, but WELL WITHIN PEX_MAX_ENTRY_AGE (1800 s) of the entries' `last_seen` so B
        // and C are still advertisable (a larger offset would age them out and suppress the `added`).
        pex.tick(1_100_000);
    ...
    fn an_unregister_surfaces_as_a_dropped_delta_on_tick() {
        ...
        assert!(
            told.contains(&b) && told.contains(&c),
            "snapshot told A of B and C"
        );

        // C leaves → next tick must drop it on A's link.
        pex.on_unregister(&c, "net");
        // Past the interval + max jitter, but within PEX_MAX_ENTRY_AGE so B stays advertisable and
        // only the departed C is dropped.
        pex.tick(1_100_000);
    ...
    fn inbound_node_pex_data_is_never_folded_into_the_introducer_set() {
        ...
        assert_eq!(
            pex.known_count("net"),
            2,
            "only the two registrants are known"
        );
        ...
        let node_snapshot = PexMessage::PexSnapshot {
            peers: vec![
                PeerEntry::new(injected.clone(), "net", 1000, Provenance::Direct)
        ...
        assert!(
            replies.is_empty(),
            "a well-formed node snapshot yields no error reply"
        );
        ...
        assert_eq!(
            b_told,
            vec![a.clone()],
            "B learns only A — never the injected peer"
        );
    ...
    fn tick_only_routes_to_handshaked_links() {
        ...
        assert!(
            drain(&mut arx)
                .iter()
                .any(|m| matches!(m, PexMessage::PexDelta { .. })),
    ...
    fn an_unsupported_version_handshake_mutes_and_only_returns_an_error() {
        ...
        assert_eq!(
            out.len(),
            1,
            "no handshake/snapshot to an incompatible peer"
        );
        match &out[0] {
