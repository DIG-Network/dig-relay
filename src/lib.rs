...
//!
//! Layering: [`wire`] is the vendored relay wire types + the RLY-008 PEX message; [`config`] is pure
//! validated configuration; [`registry`] is the in-memory peer registry + pure routing decisions;
//! [`pex`] embeds the `dig-pex` `PexEngine` for the introducer PEX binding (registry mirroring +
//! per-network scoping + the introducer-only discard rule); [`server`] is the WebSocket accept loop
//! + per-connection task + the pure `RelayMessage` dispatcher + the PEX housekeeping tick; [`stun`]
//! is the RFC 5389 STUN Binding responder (UDP) that tells a node its reflexive address; [`health`]
//! is the load-balancer HTTP probe; [`service`] installs/controls the relay as an OS service
